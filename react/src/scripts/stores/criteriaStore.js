...
var criteriaStore = Reflux.createStore({
  ...
  syncSaveLastByName(name) {
    ...
    window.rga.event({
      ...
      action: `saved_search: save: ${this.all[name] ? 're-save' : 'new-save'}`,
      label: name
    })
...
function get_verse_data(subject_type, subject_id, sliceBy, include) {
  ...
  if (result) {
    ...
    req.kiva.ajax.get('getSuperGraphData', {
        sliceBy,
        include,
        measure: 'count',
        subject_id,
      ...
      .done(result => {
        ...
        if (result.data) {
          ...
          slices = result.data.select(d => {
            return {
              id: d.name,
              name: result.lookup[d.name],
              value: parseInt(d.value),
              percent: (parseInt(d.value) * 100) / total_sum
            }
          })
        }

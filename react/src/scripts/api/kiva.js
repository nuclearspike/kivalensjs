...
class QueuedActions {
  ...
  init(options) {
    var defaults = {
      action: () => {
      }, isReady: () => true, maxQueue: 10, waitFor: 5000
    }
...
class Loans {
  constructor(update_interval) {
    ...
    this.queue_to_refresh = new QueuedActions().init({
      action: this.refreshLoans.bind(this),
      isReady: this.isReady.bind(this),
      waitFor: 5000
    })
    this.queue_new_loan_query = new QueuedActions().init({
      action: this.newLoanNotice.bind(this),
      isReady: this.isReady.bind(this),
      waitFor: 2000
    })
    this.is_ready = false
    ...
    return new LoanBatch(id_arr).start()
      ...
      .done(loans => { //this is ok when there aren't any
        ...
        try {
          ...
          console.log("BAD DATE !!!!!!!!!!!!!!: ", that.startupTime, e.message, loans.select(l => ({
            id: l.id,
  ...
  backgroundResync(notify) {
    ...
    const processLoans = function (loans) {
      ...
      if (notify) that.notify({backgroundResync: {state: 'done'}})
    }

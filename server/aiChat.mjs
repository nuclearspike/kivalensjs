...
const CRITERIA_PARAM = {
  ...
  description:
    ...
    'FULL EXAMPLE: {"loan":{"sector":"Agriculture","country_code":"UG,GH,CD,TJ,ML","percent_female_min":50,"percent_female_max":100,"sort":"popularity"},"partner":{},"portfolio":{"exclude_portfolio_loans":"true"}}. ' +
    'portfolio.exclude_portfolio_loans ("true"/"false") hides loans the lender already funded; it needs a lender id and is on by default. ' +
...
export function buildSystemPrompt(state, lenderId, criteria, extra = {}) {
  ...
  const lines = [
    ...
    'SEARCH IS LIVE — there is NO search / apply / submit / go button anywhere. Results update instantly as criteria change and are already showing on the page (the results list on the left). If the user asks how to run, start, or submit the search, or how to apply the criteria/filters, tell them it already ran and the results are on screen (quote the current match count from CONTEXT when it is present), call point_at {target:"results"} with a short callout so they see exactly where, and offer to refine — never describe an apply step and never tell them to click or press anything to "run" it.',
